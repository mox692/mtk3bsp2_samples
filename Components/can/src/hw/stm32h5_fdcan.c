/*
 * stm32h5_fdcan.c - Hardware backend for the STM32H5 FDCAN (e.g. H533).
 *
 * This is the physical layer: it knows the STM32H5 FDCAN register map and its
 * FIXED (STM32G4-style) message RAM layout.  It does NOT touch GPIO, clocks
 * or the CAN transceiver - those belong to the board support layer, which is
 * responsible for enabling the FDCAN kernel clock before can_init() runs.
 *
 * On the STM32H5 the message RAM has a hard-wired layout, so there are no
 * RXESC/TXESC/RXF0C/SIDFC registers to configure; RX filtering is done via
 * the single RXGFC register.  The H723 backend (added in Phase 2) differs
 * substantially here, which is exactly why the message-RAM handling lives
 * behind this ops table.
 *
 * Register access uses the device's reg_base / msgram_base so the same code
 * could serve another STM32H5 board or FDCAN instance without editing.
 */
#include <can/can.h>
...
/* --- FDCAN register offsets from the instance base (RM0481) --- */
...
#define FDCAN_RXGFC_OFF		0x080u	/* global filter cfg*/
#define FDCAN_RXF0S_OFF		0x090u	/* RX FIFO0 status  */
#define FDCAN_RXF0A_OFF		0x094u	/* RX FIFO0 ack     */
#define FDCAN_TXBC_OFF		0x0C0u	/* TX buffer config */
#define FDCAN_TXFQS_OFF		0x0C4u	/* TX FIFO/Q status */
#define FDCAN_TXBAR_OFF		0x0CCu	/* TX buffer add req*/
...
#define FDCAN_TXFQS_TFQPI_MASK	0x3u		/* put index (3 elements)      */

#define FDCAN_RXF0S_F0FL_MASK	0xFu		/* fill level                  */
#define FDCAN_RXF0S_F0GI_SH	8u
#define FDCAN_RXF0S_F0GI_MASK	0x3u		/* get index (3 elements)      */

#define FDCAN_PSR_LEC_MASK	0x7u
#define FDCAN_PSR_EP		(1u << 5)
#define FDCAN_PSR_BO		(1u << 7)

/* --- Fixed message RAM layout (byte offsets from msgram base, RM0481) --- */
...
/* ------------------------------------------------------------------------ */
static can_result_t h5_init(can_device_t *dev, const can_bit_timing_t *t,
			    const can_config_t *config)
{
	...
	/*
	 * Accept every frame into RX FIFO0.  RXGFC=0 means: no acceptance
	 * filters, non-matching std/ext frames go to FIFO0, remote frames
	 * accepted.
	 */
	reg_write(base + FDCAN_RXGFC_OFF, 0u);

	/* TX in FIFO mode (TFQM=0); RAM size/start are hard-wired on the H5. */
	reg_write(base + FDCAN_TXBC_OFF,
		  reg_read(base + FDCAN_TXBC_OFF) & ~(1u << 24));

	return CAN_OK;
}

static can_result_t h5_start(can_device_t *dev)
{
	uint32_t base = dev->reg_base;
	uint32_t spin;
...
static can_result_t h5_stop(can_device_t *dev)
{
	uint32_t base = dev->reg_base;
	uint32_t spin;
...
static can_result_t h5_try_send(can_device_t *dev, const can_frame_t *frame)
{
	uint32_t base = dev->reg_base;
	...
	if ((txfqs & FDCAN_TXFQS_TFQF) != 0u) {
		return CAN_ERR_BUSY;	/* TX FIFO full */
	}
...
static can_result_t h5_try_recv(can_device_t *dev, can_frame_t *frame)
{
	uint32_t base = dev->reg_base;
...
static can_result_t h5_get_status(can_device_t *dev, can_status_t *status)
{
	uint32_t base = dev->reg_base;
...
/* ------------------------------------------------------------------------ */
const can_hw_ops_t can_hw_stm32h5_fdcan_ops = {
	.init       = h5_init,
	.start      = h5_start,
	.stop       = h5_stop,
	.try_send   = h5_try_send,
	.try_recv   = h5_try_recv,
	.get_status = h5_get_status,
};
